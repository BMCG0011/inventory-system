...
export default function NestingLocation({
  initialPath = [],
  onSelect = undefined,
}: {
  ...
  const [parentLocation, selectedId, selectedName] = useMemo<
    [GetSingleLocationOutput, string | null, string | null]
  >(() => {
    ...
    return [
      selection[selection.length - 2] ?? null,
      lastSelected?.id ?? "",
  ...
  return (
    <LocationSelector
      ...
      listPrefix={
        parentLocation && (
          <CommandItem onSelect={() => setSelection(selection.slice(0, -1))}>
